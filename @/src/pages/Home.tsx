...
export default function Home() {
  ...
  const trend = useMemo(() => {
    ...
    const base = t[window];
    // remap labels so offset shifts the time window backwards without changing values
    const now = new Date();
    return base.map((pt, i) => {
      const len = base.length;
      if (window === 'monthly') {
        // weekly buckets: each point = 1 week
        const d = new Date(now);
        const weeksBack = (len - 1 - i) + trendOffset * len;
        d.setDate(d.getDate() - weeksBack * 7);
        return { label: formatDateLabel(d), value: pt.value };
      }
      if (window === 'quarterly') {
        // monthly buckets: each point = 1 month
        const d = new Date(now.getFullYear(), now.getMonth() - ((len - 1 - i) + trendOffset * len), 1);
        return { label: formatMonthYearLabel(d), value: pt.value };
      }
      // yearly
      const d = new Date(now.getFullYear(), now.getMonth() - ((len - 1 - i) + trendOffset * len), 1);
      return { label: formatMonthYearLabel(d), value: pt.value };
    });
  }, [activeTeamId, window, trendOffset]);
    ...
    <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <div
                ...
                onWheel={(e) => {
                  e.preventDefault();
                  if ((e as WheelEvent).deltaY > 0) setTrendOffset((o) => o + 1);
                  else setTrendOffset((o) => Math.max(0, o - 1));
                }}
              >
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
                  {trendOffset > 0 && <div style={{ fontSize: 12, color: C.textDim, marginRight: 8 }}>Viewing {trendOffset} period(s) ago</div>}
                </div>
                <TeamTrendChart series={trendSeries} labels={trend.map((t) => t.label)} />
              </div>
      ...
      <Section title="Team health">
        <div style={{
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
              {trendOffset > 0 && <div style={{ fontSize: 12, color: C.textDim }}>Viewing {trendOffset} period(s) ago</div>}
            </div>
      ...
      {/* Team & motivations removed from team pages per request */}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ boxSizing: 'border-box', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, padding: 22, width: '100%', minWidth: 420, minHeight: 420, display: 'block', color: C.textDim, overflow: 'visible' }}>
                {sheetMemberId ? (
                  (() => {
                    const member = teamMembers.find((m) => m.id === sheetMemberId)!;
                    const personDrivers = memberDrivers[sheetMemberId] || teamAverageDrivers;
                    const sorted = [...driverNames].sort((a, b) => personDrivers[b] - personDrivers[a]);
                    const top = sorted.slice(0, 2);
                    const bottom = sorted.slice(-2).reverse();
                    const spots = blindSpots[sheetMemberId] || [];
                    return (
                      <div style={{ width: '100%' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
                          <MotivationRadar
                            size={360}
                            series={[
                              { label: member.name.split(' ')[0], values: personDrivers, colour: member.colour || '#C2F542' },
                              { label: 'Team avg', values: teamAverageDrivers, colour: '#A0E0FF', fill: '#A0E0FF' },
                            ]}
                          />
                        </div>

                        {/* New container below the graph containing the info cards */}
                        <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
                          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12 }}>
                            <div style={{ fontSize: 10, letterSpacing: 1.2, color: C.textDim, marginBottom: 8 }}>DOMINANT</div>
                            {top.map((d) => (
                              <div key={d} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '4px 0', color: C.text }}>
                                <span>{d}</span><span style={{ color: '#C2F542', fontWeight: 600 }}>{personDrivers[d]}</span>
                              </div>
                            ))}
                          </div>

                          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12 }}>
                            <div style={{ fontSize: 10, letterSpacing: 1.2, color: C.textDim, marginBottom: 8 }}>LOWEST</div>
                            {bottom.map((d) => (
                              <div key={d} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '4px 0', color: C.text }}>
                          ...
                          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 12 }}>
                            <div style={{ fontSize: 10, letterSpacing: 1.2, color: '#FFB28A', marginBottom: 8 }}>BLIND SPOTS</div>
                            {spots.length === 0 ? (
                              <div style={{ color: C.textDim, fontSize: 13 }}>No blind spots detected.</div>
                            ) : (
                              spots.map((s, i) => (
                                <div key={i} style={{ fontSize: 13, color: C.text, lineHeight: 1.55, padding: i === 0 ? '0' : '8px 0', borderTop: i === 0 ? 'none' : `1px solid ${C.border}` }}>{s}</div>
                              ))
                            )}
                          </div>
                        </div>
